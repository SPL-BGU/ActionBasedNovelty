...
class Planner:
    ...
    def solve(self, domain, problem):
        ...
        print(
            "\t* model parse time: "
            + str("{:5.4f}".format(time.time() - start_solve_time))
            + "s"
        )
        print("\n=================================================\n\n\n\n\n\n\n")
        # Do nothing
        ...
        while self.queue:
            ...
            if grounded_instance.goals(state, constants):
                ...
                if grounded_instance.problem.metric == ["total-time"]:
                    ...
                    grounded_instance.problem.metric == ["total-actions"]
                    or grounded_instance.problem.metric == None
                ):
            ...
            for aa in applicables:
                ...
                if aa == constants.TIME_PASSING_ACTION:
                    ...
                    if constants.DOUBLE_EVENT_CHECK:
                        ...
                        happenings_list = new_state.get_applicable_happenings(
                            grounded_instance.events
                        )
                        for hp_e1 in happenings_list:
                            new_state = new_state.apply_happening(
                                hp_e1,
                    ...
                    new_state.time = constants.fast_round(
                        state.time + constants.DELTA_T, constants.NUMBER_PRECISION
                    )
                    ...
                    new_state = state.apply_happening(
                        aa, from_state=from_state, create_new_state=True
                    )
                ...
                if (
                    new_state.time <= constants.TIME_HORIZON
                    and new_state.depth <= constants.DEPTH_LIMIT
                    and grounded_instance.duration_constraints(new_state, constants)
                ):
                    if (
                        (new_state_hash not in self.visited_hashmap)
                        or (
                            constants.METRIC_MINIMIZE
                            and new_state.metric
                            < self.visited_hashmap[new_state_hash].state.metric
                        )
                        or (
                            not constants.METRIC_MINIMIZE
                            and new_state.metric
                            > self.visited_hashmap[new_state_hash].state.metric
                        )
                    ):
                ...
                if self.explored_states % constants.PRINT_INFO == 0:
                    ...
                    print_q.append(
                        "["
                        + str("{:6.2f}".format(time_checkpoint))
                        + "] ==> states explored: "
                    ...
                    print_q.append(
                        "\t\t ==> exploration rate: "
                        + str(
                            constants.fast_round(
                                self.explored_states / time_checkpoint, 2
                            )
                        )
                        + " states/sec"
                    )
                    if constants.ANYTIME:
                        print_q.append(
                            "\t\t ==> tracked goals: "
                            + str(len(self.reached_goal_states))
                        )
                        print_q.append(
                            "\t\t ==> total goals found: " + str(self.total_goals_found)
                        )
                        if len(self.reached_goal_states) > 0:
                            print_q.append(
                                "\t\t ==> best metric: {:6.3f}".format(
                                    self.reached_goal_states[0].state.metric
                                )
                            )
                        else:
        ...
        while self.queue:
            ...
            for aa in applicables:
                ...
                if aa == constants.TIME_PASSING_ACTION:
                    ...
                    # first check for triggered events if '-dblevent' flag is true
                    if constants.DOUBLE_EVENT_CHECK:
                        happenings_list = grounded_instance.events.get_applicable(
                            new_state
                        )
                        for hp_e1 in happenings_list:
                            new_state = new_state.apply_happening(
                    ...
                    # next check applicable processes
                    happenings_list = grounded_instance.processes.get_applicable(
                        new_state
                    )
                    for hp_p1 in happenings_list:
                        new_state = new_state.apply_happening(
                ...
                if (
                    ...
                    if (
                        (new_state_hash not in self.visited_hashmap)
                        or (
                            new_state_hash in self.visited_hashmap
                            and constants.METRIC_MINIMIZE
                            and new_state.metric
                            < self.visited_hashmap[new_state_hash].state.metric
                        )
                        or (
                            new_state_hash in self.visited_hashmap
                            and not constants.METRIC_MINIMIZE
                            and new_state.metric
                            > self.visited_hashmap[new_state_hash].state.metric
                        )
                    ):
    ...
    def enqueue_goal(self, n_state):
        ...
        if constants.METRIC_MINIMIZE:
            if (len(self.reached_goal_states) < constants.TRACKED_PLANS) or (
                (len(self.reached_goal_states) == constants.TRACKED_PLANS)
                and (
                    (n_state.state.metric < self.reached_goal_states[-1].state.metric)
                    or (
                        n_state.state.metric
                        == self.reached_goal_states[-1].state.metric
                        and n_state.state.depth
                ...
                self.reached_goal_states = collections.deque(
                    sorted(
                        ...
                        key=lambda elem: (elem.state.metric, elem.state.depth),
                    ),
    ...
    def get_trajectory(self, sstate: State):
        ...
        while curr_v_state.state.predecessor_action is not None:
            ...
            plan.insert(
                0,
                (curr_v_state.state.predecessor_action, copy.copy(curr_v_state.state)),
            )
