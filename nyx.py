...
def process_arguments(cl_arguments):
    ...
    for arg in cl_arguments:
        ...
        if arg_list[0] == "config":
            ...
            constants.TIME_PASSING_ACTION.duration = round(
                constants.DELTA_T, constants.NUMBER_PRECISION
            )
        elif arg_list[0] == "tv" or arg_list[0] == "track_variables":
            constants.TRACK_VARIABLES = True
            constants.TRACKED_VARIABLES_LIST = (
                arg_list[1].replace("[", "").replace("]", "").split(",")
            )
            constants.VERY_VERBOSE_OUTPUT = True
            ...
            print(
                "\nERROR: Unrecognized Argument: {}\nCall with -h flag for help".format(
                    arg
                )
            )
            exit(1)
...
def print_config(dom, prob):
    config_string = (
        ...
        + str(prob)
        + "\n\t* config file: "
        + str(constants.CONFIG_FILE)
        + "\n\t* search algorithm: "
        + str(constants.SEARCH_ALGO_TXT)
        + "\n\t* depth limit: "
        + str(constants.DEPTH_LIMIT)
        + "\n\t* timeout: "
        + str(constants.TIMEOUT)
        + "s"
    )
    if constants.TEMPORAL_DOMAIN:
        config_string += (
            "\n\t* time discretisation: "
            + str(constants.DELTA_T)
            + "\n\t* time horizon: "
            + str(constants.TIME_HORIZON)
        )
    print(config_string + "\n")
...
def print_solution_info(goal_st, plnr, sg_time, ttime):
    ...
    config_string = (
        ...
        + str(round(ttime, 3))
        + "\n\t* explored states: "
        + str(plnr.explored_states)
        + "\n\t* plan length: "
        + str(non_temporal_count)
        + " ("
        + str(len(pln))
        + ")"
        + "\n\t* plan duration: "
...
def runner(dom_file, prob_file, args_list: []):
    ...
    if not my_plnr.reached_goal_states:
        ...
        if (
            constants.TRACK_VARIABLES or constants.PLOT_TRACKED_VARIABLES
        ) and constants.TRACKED_VARIABLES_LIST == []:
            constants.TRACKED_VARIABLES_LIST = list(
                my_plnr.reached_goal_states[0].state.state_vars.keys()
            )
            constants.TRACKED_VARIABLES_LIST = [
                s.replace("[", "").replace("]", "").replace("'", "").replace(",", "")
                for s in constants.TRACKED_VARIABLES_LIST
            ]
        ...
        for sg in my_plnr.reached_goal_states:
            ...
            one_plan_file = (
                plan_path + "plan" + str(count_gs) + "_" + os.path.basename(prob_file)
            )
            ...
            if constants.TRACK_VARIABLES or constants.PLOT_TRACKED_VARIABLES:
                extract_variable_values(
                    one_plan_file,
                    constants.TRACKED_VARIABLES_LIST,
                    os.path.splitext(one_plan_file)[0] + "_tracked_vars",
                )
        ...
        print(
            "\t\tBest Plan Metric: {:6.3f}".format(
                my_plnr.reached_goal_states[0].state.metric
            )
        )
        print("\n=================================================\n")
...
def process_goal(plan_obj, planner_obj, plan_save_file):
    ...
    for pair in plan_obj:
        ...
        if (not (constants.VERBOSE_OUTPUT or constants.VERY_VERBOSE_OUTPUT)) and pair[
            0
        ] == constants.TIME_PASSING_ACTION:
            continue
        ...
        str1 = (
            ...
            + str(
                "{:10.3f}".format(
                    planner_obj.visited_hashmap[pair[1].predecessor_hashed].state.time
                )
            )
            + ":\t"
            + str(pair[0].name)
        ...
        str2 = (
            str(pair[0].parameters).replace("'", "").replace(",)", ")")
...
def extract_variable_values(plan_f, var_list, csv_f=None):
    ...
    csv_rows = [
        list(x) for x in zip(*csv_rows_untransposed)
    ]  # transpose the tracked values matrix
    ...
    if constants.PLOT_TRACKED_VARIABLES:
        ...
        for i in range(1, len(csv_rows_untransposed)):
            ...
            if constants.PLOT_NUMERIC and (type(csv_rows_untransposed[i][0]) is bool):
                continue
            if constants.PLOT_BOOLEAN and not (
                type(csv_rows_untransposed[i][0]) is bool
            ):
                continue

            if (
                type(csv_rows_untransposed[i][0]) is bool
            ) and not constants.PLOT_BOOLEAN:
                axes.plot(
                    csv_rows_untransposed[0],
                    csv_rows_untransposed[i],
                    linestyle="--",
                    label=csv_vars[i],
                )
            else:
